      ...
      <YMaps>
        <MapContainer>
          <YMap
            ...
            width={900}
            height={800}
            defaultState={{ center: [lat, long], zoom: 12 }}
          >
            <>
              {Object.keys(postsByCoord).map((key) => {
                const [lat, long] = key.split('_');

                const shouldShowPlacemark = postsByCoord[key].some(
                  (post) => post.content?.coord?.isUsedGeo
                );
          ...
          {postsByCoord[currentKey] && (
            <Modal active={isModalActive} setActive={setModalActive}>
              {currentKey &&
                postsByCoord[currentKey].map((post) => {
